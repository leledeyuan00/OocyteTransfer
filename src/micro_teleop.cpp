  ...
  a_scale_(2.0)
{
  ...
  x_axis_pub = nh_.advertise<std_msgs::Float64>("/joint3_position_controller/command", 1);
  y_axis_pub = nh_.advertise<std_msgs::Float64>("/joint2_position_controller/command", 1);
  z_axis_pub = nh_.advertise<std_msgs::Float64>("/joint1_position_controller/command", 1);
...
void TeleopTurtle::keyLoop()
{
  ...
  for(;;)
  {
    ...
    if(dirty ==true)
    {
      ...
      // z.data = z_cmd;
      // z_axis_pub.publish(z);  

      ROS_INFO("CMD IS %f",x.data);
      dirty=false;
    }
  }
